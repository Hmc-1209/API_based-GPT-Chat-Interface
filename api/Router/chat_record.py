...
from Repository.ChatRecordCRUD import create_new_chat_record, get_all_chat_record_names, get_chat_record_content, delete_chat_record_content
...
async def get_all_chat_record(current_user=Depends(get_current_user)) -> list[ReadChatRecord]:
    ...
    records = await get_all_chat_record_names(current_user.user_id)
